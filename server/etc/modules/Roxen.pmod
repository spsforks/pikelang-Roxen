...
constant magic_charset_variable_value = "���&#x829f;@" + magic_charset_variable_placeholder;
...
string get_client_charset (string ���)
...
//! @[get_client_charset_decoder], @[get_decoder_for_client_charset]
{
  ...
  //  http_decode_string().
  if (has_prefix(���, "%") && !has_prefix(���, "%%")) {
    report_notice("Warning: Double HTTP encoding detected: %s\n", ���);
    string cs = get_client_charset (http_decode_string(���));
    if (cs) {
      return cs + "|http";
    } else {
      return "http";
...
function(string:string) get_client_charset_decoder( string ���,
                                                    RequestID|void id )
...
//! @[get_client_charset]
{
  string charset = get_client_charset (���);
...
string get_wizard_id_cookie_name(RequestID id)
{
  bool secure = id->client_scheme && id->client_scheme() == "https";
...
string get_wizard_id_cookie(RequestID id)
{
  return id->cookies[get_wizard_id_cookie_name(id)];
}
